...
modules = {"1": "months.csv", "2": "rutina.csv", "3": "verbs.csv", "4": "colours.csv", "5": "face.csv"}
module_names = {"1": "Meses del año en español", "2": "Rutina días", "3": "Verbos Regulares", "4": "Los Colores",
                "5": "Partes de la cara"}
...
def complexAnsCheck(ans, a, ans_format):
    ...
    for i, j in enumerate(a):
        ...
        if i in special_types:
            ...
            print("word doesn't match", repr(j), repr(split_ans[i]))
            check = False
            break
    if check:
        print(okay)
...
def main():
    ...
    while ans != "quit" and ans != "q":  # Ask Questions till user doesn't want to stop
        ...
        if not rand_row["Type"] in questions.keys():
            raise TypeError(
                "Unknown Type of question found. Please refer to the manual for the acceptable question types.")
        question_infos = questions[rand_row["Type"]](rand_row)
        q, a = question_infos[0], question_infos[1]
        ans = str(input(q + "\n--> "))
